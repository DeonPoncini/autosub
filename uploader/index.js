...
function getS3Client () {
  console.log(process.env.COMPANION_AWS_KEY);
  console.log(process.env.COMPANION_AWS_SECRET);
...
function isValidPartDebug(part) {
    console.log(part)
    if (!part) {
        console.log("Part is not defined");
        return false;
    }
    if (typeof part !== 'object') {
        console.log("Part is not an object");
        return false;
    }
    if (!Number(part.PartNumber)) { console.log("Part is not a part number");
        return false;
    }
    if (typeof part.ETag !== 'string') {
        console.log("ETag is not a string");
        return false;
    }
    return true;
}

function convertPartsArray(parts) {
    const ret = []
    for (var i = 0; i < parts.length; i++) {
        const part = parts[i];
        for (var p = 0; p < part.PartNumber.length; p++) {
            const pn = part.PartNumber[p];
            const etag = part.ETag[p];
            ret.push({
                PartNumber: pn,
                ETag: etag,
            });
...
app.post('/s3/multipart/:uploadId/complete', (req, res, next) => {
  ...
  if (typeof key !== 'string') {
      console.log("Returning 400 string");
    return res.status(400).json({ error: 's3: the object key must be passed as a query parameter. For example: "?key=abc.jpg"' })
  }
  if (!Array.isArray(parts)) {
    ...
    // convert the parts into a proper side by side array
  console.log(parts);
  const n_parts = convertPartsArray(parts);
  console.log(n_parts);
  if (!Array.isArray(n_parts) || !n_parts.every(isValidPartDebug)) {
    return res.status(400).json({ error: 's3: `parts` must be an array of {ETag, PartNumber} objects.' })
  }
